...
namespace KtsDBHelper
{
        ...
        *****************************************/
            ...
            public static void WriteFile(string Path, string Strings,bool Append)
            {
        ...
        *****************************************/
            ...
            public static void WriteFile(string Path, string Strings, bool Append,string encode)
            {
                ...
                System.IO.StreamWriter f2 = new System.IO.StreamWriter(Path, Append, System.Text.Encoding.GetEncoding(encode));
                f2.WriteLine(Strings);
                f2.Close();
                f2.Dispose();
        ...
        *****************************************/
            ...
            public static void CopyDir(string srcPath, string aimPath)
            {
                ...
                {
                    KtsDBHelper.Log.WriteExLog(ee + "", typeof(FileObj));
                }
            }
            #endregion
        ...
        *****************************************/
            ...
            /// <summary>
            /// 递归目录和子目录文件返回的List（需要先调用FindFile(string path)）
            /// </summary>
            public static List<string> al = new List<string>();
            /// <summary>
            /// 递归目录和子目录文件
            /// </summary>
            ...
            public static void FindFile(string path)
            {
                ...
                try
                {
                    ...
                    {
                        //是目录的话(那当前子目录重新调用该方法递归)
                        FindFile(dir.ToString() + "\\" + childir.ToString());
                    }
                    foreach (FileInfo fi in dir.GetFiles())
                    {   //是文件的话直接添加到 ArrayList al 
                        al.Add(dir.ToString() + "\\" + fi.ToString() + "\n");
                    }
                }
                catch(Exception ex)
                {
                    KtsDBHelper.Log.WriteExLog(ex+"", typeof(FileObj));
                }
            }
            ...
            {
                foreach (string a in Directory.GetFileSystemEntries(path))
                {
                    if (File.Exists(a))
                    {
            ...
            /// <summary>
            /// copy目录下所有文件到另一目录,包括子目录和下面文件
            /// </summary>
            /// <param name="yanfilepath">源地址</param>
            /// <param name="mudifilepath">目的地址</param>
            public static void CopyFilesDirs(string yanfilepath, string mudifilepath)
            {
                ...
                {
                    Directory.CreateDirectory(mudifilepath);
                    for (int i = 0; i < arrFiles.Length; i++)
                    {
                        File.Copy(yanfilepath + "\\" + Path.GetFileName(arrFiles[i]), mudifilepath + "\\" + Path.GetFileName(arrFiles[i]), true);
                    }
                }
                if (arrDirs.Length != 0)
                {
                    ...
                    {
                        string srcchilddir = yanfilepath + "\\" + Path.GetFileName(arrDirs[i]);//  
                        string dstchilddir = mudifilepath + "\\" + Path.GetFileName(arrDirs[i]);
                        string srcdirname = yanfilepath.Substring(yanfilepath.LastIndexOf("\\"));
                        string dstdirname = mudifilepath.Substring(mudifilepath.LastIndexOf("\\"));
                        if (!srcdirname.Equals(dstdirname))//如果源文件夹名字和目的文件夹名字相同，不拷贝，不然生成死循环，每次打开自己备份到自己。
                        {
                            Directory.CreateDirectory(dstchilddir);
                            CopyFilesDirs(srcchilddir, dstchilddir);//递归调用 
                        }
                    }
                }
